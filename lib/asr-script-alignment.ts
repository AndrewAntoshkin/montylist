...
const MIN_FUZZY_SIMILARITY = 0.5;   // СНИЖЕН ещё больше для лучшего покрытия коротких реплик
const ANCHOR_SIMILARITY = 0.8;     // СНИЖЕН для большего количества якорей
const FORCED_ALIGNMENT_SIMILARITY = 0.9; // СНИЖЕН для принудительного матча
const MAX_SCRIPT_JUMP = 30;        // УВЕЛИЧЕН для более гибкого поиска (было 20)
