...
export function formatCharactersForPromptV4(
  ...
  const lines: string[] = [
    ...
    '═══════════════════════════════════════════════════════',
    '',
    '🚨 КРИТИЧЕСКИ ВАЖНО ДЛЯ ИМЁН:',
    '',
    '1. ИСПОЛЬЗУЙ ТОЛЬКО ИМЕНА ИЗ СПИСКА НИЖЕ!',
    '   • НЕ заменяй одно имя на другое похожее',
    '   • НЕ путай разных персонажей с похожими именами',
    '   • Каждое имя в списке — УНИКАЛЬНЫЙ персонаж!',
    '',
    '2. АЛГОРИТМ ОПРЕДЕЛЕНИЯ ПЕРСОНАЖА:',
    '   а) Смотри ТИТРЫ в видео — там указаны имена актёров и персонажей',
    '   б) Сравни внешность с ОПИСАНИЯМИ в списке ниже',
    '   в) Используй ТОЧНО то имя, которое в списке',
    '',
    '3. Если НЕ МОЖЕШЬ определить имя → используй РОЛЬ:',
    '   • КОСМЕТОЛОГ (делает маски, процедуры)',
    '   • МАСТЕР МАНИКЮРА (рисует хной, красит ногти)',
    '   • ОФИЦИАНТ (с подносом, принимает заказ)',
    '   • ВРАЧ, МЕНЕДЖЕР, и т.д.',
    '',
    '4. КАЖДАЯ РЕПЛИКА ДОЛЖНА ИМЕТЬ СПИКЕРА!',
    '   • Знаешь имя → используй имя из списка',
    '   • Не знаешь → используй роль (КОСМЕТОЛОГ, ОФИЦИАНТ)',
    '   • За кадром → добавь ЗК (ИМЯ ЗК или РОЛЬ ЗК)',
  ];
  
  lines.push('');
  lines.push('🎯 КАК ОПРЕДЕЛИТЬ КТО ГОВОРИТ:');
  lines.push('   1. Смотри чьи ГУБЫ двигаются синхронно с речью');
  lines.push('   2. Если губы НЕ двигаются у человека в кадре → это ЗК!');
  lines.push('   3. Найди персонажа по описанию ниже');
  lines.push('');
  lines.push('🔇 ГОЛОС ЗА КАДРОМ (ЗК):');
  lines.push('   • Камера на одном персонаже, а говорит ДРУГОЙ');
  lines.push('   • Пример: в кадре Галя (молчит), голос Томы → ТОМА ЗК');
  
  if (main.length > 0) {
    lines.push('');
    lines.push('👥 ГЛАВНЫЕ ПЕРСОНАЖИ (используй описания для визуальной идентификации):');
  ...
  lines.push('');
  lines.push('🚨🚨🚨 РАЗРЕШЁННЫЕ ИМЕНА (ИСПОЛЬЗУЙ ТОЛЬКО ЭТИ!):');
  lines.push(`   ${allNames}`);
  lines.push('');
  lines.push('⛔ ЗАПРЕЩЕНО:');
  lines.push('   • Использовать имена НЕ из этого списка');
  lines.push('   • Заменять одно имя на похожее');
  lines.push('   • Придумывать новые имена');
  lines.push('═══════════════════════════════════════════════════════');
...
0. 🔊 КОРОТКИЕ РЕПЛИКИ — НЕ ПРОПУСКАЙ!
   - Даже планы 1-2 секунды могут содержать короткие фразы: "Что?", "Ага", "По...", "Минуточку"
   - СЛУШАЙ ВНИМАТЕЛЬНО — если слышна речь, пиши её!
   - НЕ пиши "Музыка" если есть хоть одно слово!
   - Пример коротких реплик: "Как?", "Ой", "Да", "Нет", "Почему?", "Эй!"
