...
export function formatCharactersForPromptV4(
  ...
  const lines: string[] = [
    ...
    '---',
    '',
    '⚠️ КРИТИЧЕСКИ ВАЖНО — ЗАСТАВКА И ТИТРЫ:',
    '',
    'В первые 1-2 минуты фильма идёт заставка с титрами актёров и персонажей.',
    'Формат титров: "ИМЯ_ПЕРСОНАЖА — Имя Актёра" или "Имя Актёра — ИМЯ_ПЕРСОНАЖА"',
    '',
    'Примеры титров (запомни их для идентификации позже!):',
    '- "Шурочка — Татьяна Рыбинец" → персонаж ШУРОЧКА',
    '- "Тома — Елена Доронина" → персонаж ТОМА (НЕ Татьяна, НЕ Тамара!)',
    '- "Надежда — Галина Ступакова" → персонаж НАДЯ (актриса Галина, но персонаж НАДЯ!)',
    '- "Галина — Полина Нечитайло" → персонаж ГАЛЯ (актриса Полина, но персонаж ГАЛЯ!)',
    '',
    'Правила определения персонажей:',
    '',
    '1. Используй ТОЛЬКО имена из списка ниже.',
    '2. В заставке — внимательно читай ВСЕ титры и запоминай лица!',
    '3. Актёр и персонаж — РАЗНЫЕ имена! Используй имя ПЕРСОНАЖА.',
    '4. Если у персонажа есть описание — сравни с тем, что видишь в кадре.',
    '5. НЕ пиши роль (Косметолог, Официант) — всегда ищи имя!',
    '6. За кадром — добавь ЗК (например: ТОМА ЗК)',
    '',
    'Как определить кто говорит:',
    '- Смотри чьи губы двигаются синхронно с речью',
    '- Если человек в кадре НЕ говорит — это ЗК от предыдущего спикера',
    '- Слушай как персонажи обращаются друг к другу (Танька! Галь! Юсеф!)',
    '  → Если слышишь "Таненька" — значит в сцене НАДЯ или ТАНЯ',
    '  → Если слышишь "Томка" — значит в сцене ТОМА',
    '',
    'Голос за кадром (ЗК):',
    '- Камера на слушающем, а говорит другой — пиши ИМЯ_ГОВОРЯЩЕГО ЗК',
  ];
  ...
  if (main.length > 0) {
    ...
    for (const c of main.slice(0, 12)) {
      ...
      // Если нет описания — напоминаем искать в титрах
      const hint = desc ? '' : ' [ищи в титрах заставки!]';
      lines.push(`- ${c.name.toUpperCase()} ${gender}${desc ? ': ' + desc : hint}`);
    }
  }
...
3. За кадром (ЗК) — критически важно.
   ...
   - Камера на слушающем (персонаж А в кадре), а говорит персонаж Б — ПЕРСОНАЖ_Б ЗК
   - Камера на реакции (один смотрит), а говорит другой — ГОВОРЯЩИЙ ЗК
...
5. Имена — используй ТОЧНО как в списке персонажей:
   - Используй короткие формы имён если они указаны в списке
   - НЕ заменяй имена на профессии (КОСМЕТОЛОГ, МЕНЕДЖЕР, ОФИЦИАНТ)
   - Если персонаж не определён точно — пиши "ГОВОРЯЩИЙ ЗК"

6. Типы планов — ТОЧНЫЕ КРИТЕРИИ (смотри внимательно!):
   
   ✅ Кр. (Крупный) — ТОЛЬКО голова и плечи
      Признаки: видны глаза, губы, морщинки, детали лица
      Примеры: "Галя говорит в кадре", "Юсеф смотрит на Галю", "Тома кивает"
      Размер: лицо занимает >40% кадра
   
   ✅ Ср. (Средний) — от пояса/колен, видны руки и жесты
      Признаки: видно тело, руки, движения, 1-3 человека
      Примеры: "Галя садится на кушетку", "Тома и Юсеф разговаривают"
      Размер: человек занимает 20-40% кадра
   
   ✅ Общ. (Общий) — всё помещение, локация целиком
      Признаки: видна вся комната/зал, много людей ИЛИ один издалека
      Примеры: "Салон красоты", "Галя на дальнем плане, Тома на переднем"
      Размер: человек <20% кадра ИЛИ видны стены/интерьер
   
   ✅ Деталь — только объект (без лиц)
      Примеры: "Руки Гали", "Кольцо на пальце", "Дверь закрывается"
   
   ⚠️ НДП — добавляется ТОЛЬКО если видны титры на экране
   
   ПРАВИЛО: При сомнении между Кр. и Ср. → выбирай Ср.
   ПРАВИЛО: Если видно >3 людей ИЛИ весь интерьер → это Общ.

7. ВАЖНО — не пропускай планы!
   - Опиши ВСЕ ${scenes.length} таймкоды из списка
   - Даже если сцена повторяется (например, серия Кр. планов) — опиши каждую
   - Короткие планы (0.5-1 сек) тоже важны — это реакции, взгляды
...
4. Имена — ТОЧНО как в списке персонажей:
   - Используй короткие формы если указаны
   - НЕ заменяй имена на профессии
...
export function parseResponseV4(response: string): Array<{
  ...
  // 1. **00:00:00:00 - 00:00:04:01**
  // 2. **[00:00:00:00] - [00:00:04:01]**
