...
import { createPromptWithDetectedScenes } from '@/lib/gemini-prompt-with-scenes';
import { filterScenesForChunk } from '@/lib/scene-detection';
import type { DetectedScene } from '@/lib/scene-detection';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (!testResponse.ok) {
        console.warn(`⚠️  Chunk URL returned non-200 status. This may cause issues with Gemini API.`);
      }
    } catch (testError) {
    ...
    let prompt: string;
    let scenesForChunk: DetectedScene[] | null = null;
    
    // Check if we have pre-detected scenes from FFmpeg
    if (chunkProgress.useSceneDetection && chunkProgress.detectedScenes) {
      console.log(`\n🎬 Using FFmpeg detected scenes for chunk ${chunkIndex}`);
      
      // Filter scenes to this chunk's time range
      scenesForChunk = filterScenesForChunk(
        chunkProgress.detectedScenes,
        startTimecode,
        endTimecode
      );
      
      if (scenesForChunk.length > 0) {
        console.log(`✅ Found ${scenesForChunk.length} pre-detected scenes for this chunk`);
        
        // Create plan boundaries from detected scenes
        const planBoundaries = [];
        for (let i = 0; i < scenesForChunk.length - 1; i++) {
          planBoundaries.push({
            start_timecode: scenesForChunk[i].timecode,
            end_timecode: scenesForChunk[i + 1].timecode,
          });
        }
        
        // Use specialized prompt for pre-detected scenes
        prompt = createPromptWithDetectedScenes(planBoundaries, chunkIndex, totalChunks);
        console.log(`📝 Generated prompt WITH ${planBoundaries.length} detected scenes`);
      } else {
        console.warn(`⚠️ No scenes found for chunk ${chunkIndex}, falling back to standard prompt`);
        prompt = createChunkPrompt(chunkIndex, startTimecode, endTimecode, totalChunks);
        console.log(`📝 Generated standard prompt (fallback)`);
      }
    } else {
      // Standard mode: Gemini finds scenes itself
      console.log(`📝 Using standard prompt (Gemini will detect scenes)`);
      prompt = createChunkPrompt(chunkIndex, startTimecode, endTimecode, totalChunks);
    }
    ...
    if (aiResponse.length === 0 || aiResponse.trim().length === 0) {
      console.error(`❌ EMPTY RESPONSE from Gemini for chunk ${chunkIndex}!`);
    ...
    if (parsedScenes.length === 0) {
      console.warn(`⚠️  No scenes found in chunk ${chunkIndex} after trying all parsers`);
    ...
    const validScenes = parsedScenes.filter(scene => {
      ...
      if (!isWithinRange) {
        console.log(`⚠️  Scene ${scene.start_timecode} - ${scene.end_timecode} is outside chunk range, filtering out`);
      }
    ...
    if (validScenes.length < parsedScenes.length) {
      console.log(`⚠️  Filtered out ${parsedScenes.length - validScenes.length} scenes outside chunk range (${chunkStartSeconds}s - ${chunkEndSeconds}s)`);
      console.log(`✅ Kept ${validScenes.length} scenes within chunk range`);
      parsedScenes = validScenes;
    } else {
      console.log(`✅ All ${parsedScenes.length} scenes are within chunk range`);
    }
