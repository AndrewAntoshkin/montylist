...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // FFmpeg Scene Detection (OPTIONAL - controlled by env variable)
    let detectedScenes = null;
    const useSceneDetection = process.env.USE_SCENE_DETECTION === 'true';
    ...
    if (useSceneDetection) {
      ...
      if (ffmpegAvailable) {
        try {
          // Detect video FPS
          const videoFPS = await detectVideoFPS(originalVideoPath);
          
          // Detect all scenes in the video
          const threshold = parseFloat(process.env.SCENE_DETECTION_THRESHOLD || '0.3');
          const scenes = await detectScenes(originalVideoPath, {
            threshold,
            fps: videoFPS,
            maxScenes: 5000,
          });
          ...
          if (scenes.length > 0) {
            console.log(`✅ FFmpeg detected ${scenes.length} scenes successfully`);
            detectedScenes = scenes;
          } else {
            console.warn(`⚠️ FFmpeg detected 0 scenes - falling back to Gemini-only mode`);
          }
        } catch (error) {
          console.error(`❌ FFmpeg scene detection failed:`, error);
          console.log(`⚠️ Falling back to Gemini-only mode`);
        }
      } else {
        console.warn(`⚠️ FFmpeg not available - falling back to Gemini-only mode`);
      }
    } else {
      console.log(`\n🎬 FFmpeg Scene Detection is DISABLED (using Gemini-only mode)`);
      console.log(`   To enable: set USE_SCENE_DETECTION=true in .env.local`);
    }
    
    // Update chunk progress with detected scenes (if available)
    if (detectedScenes) {
      chunkProgress.detectedScenes = detectedScenes;
      chunkProgress.useSceneDetection = true;
      console.log(`✅ Added ${detectedScenes.length} detected scenes to chunk progress`);
    }
